...
from caller import single_image_converter
...
def generate_images(network_pkl, seeds, truncation_psi, mode):
    ...
    Gs_kwargs.randomize_noise = False # for disable random noise
...
def truncation_trick(network_pkl, seeds, mode, interval=0.01):
    ...
    start = -1
    stop = 1
    truncation_psi_range = np.arange(start, stop + interval, interval)
    for trun_index, truncation_psi in enumerate(truncation_psi_range):
        truncation_psi_range[trun_index] = round(truncation_psi,3)
    
    for truncation_psi in truncation_psi_range:
        if truncation_psi is not None:
            Gs_kwargs.truncation_psi = truncation_psi

        for seed_idx, seed in enumerate(seeds):
            print('Generating image for seed %d with truncation_psi %.03f (%d/%d) ...' % (seed, truncation_psi, seed_idx, len(seeds)))
            rnd = np.random.RandomState(seed)
            z = rnd.randn(1, *Gs.input_shape[1:]) # [minibatch, component]
            #tflib.set_vars({var: rnd.randn(*var.shape.as_list()) for var in noise_vars}) # [height, width]
            images_2D = Gs.run(z, None, **Gs_kwargs) # [minibatch, height, width, channel]
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    func_name_map = {
        ...
        'truncation-trick': 'run_generator.truncation_trick'
    }
